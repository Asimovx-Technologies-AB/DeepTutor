...
def _user_topic(user_id: str, topic_id: str) -> str:
    """Build a per-user namespaced topic key used for ChromaDB + graph store."""
    safe_uid = user_id.replace("-", "_")
    safe_tid = (topic_id or "general").replace("-", "_")
    return f"{safe_uid}_{safe_tid}"


async def _run_indexing(doc_id: str, topic_id: str, file_path: str, user_id: str):
    ...
    # Use user-namespaced topic so each user has their own vector collection
    namespaced_topic = _user_topic(user_id, topic_id)
...
async def upload_document(
    ...
    # Save to disk under user-specific subdirectory
    upload_dir = Path(settings.UPLOAD_DIR) / user["id"] / topic_id
...
async def get_knowledge_graph(topic_id: str, user: dict = Depends(get_current_user)):
    """Return full knowledge graph for visualization — scoped to the current user."""
    namespaced_topic = _user_topic(user["id"], topic_id)
