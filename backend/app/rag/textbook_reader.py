"""
Direct Kerala SCERT Class 10 Textbook Reader & Context Extractor.
...
# Alias mappings for common query variations
TOPIC_ALIASES = {
    "math": "sslc-math",
    "maths": "sslc-math",
...
def extract_textbook_chunks(
    ...
    if not pdf_path.exists():
        print(f"[textbook_reader] Warning: Textbook PDF not found at {pdf_path}")
        return []
    ...
    try:
        ...
        for p_idx in range(len(doc)):
            ...
            if page_num in target_pages:
                page_text = doc[p_idx].get_text("text").strip()
                if page_text and len(page_text) > 40:
                    extracted_page_texts.append((page_num, page_text))

        # Split into semantic paragraphs / chunk sections
        ...
        # Rank by query relevance if query is provided
        ...
        # If no query or even sampling, distribute chunks across the chapter pages
        ...
        print(f"[textbook_reader] Error reading {pdf_path}: {e}")
        return []
